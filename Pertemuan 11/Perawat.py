...
class Perawat:
    ...
    def update(self, id):
        ...
        sql="UPDATE perawat SET   = %s, nama = %s, jk=%s, WHERE id=%s"
    ...
    def deleteBynip(self, nip):
        self.conn = mydb()
        sql="DELETE FROM perawat WHERE id='" + str(nip) + "'"
        self.affected = self.conn.delete(sql)
        self.conn.disconnect
        return self.affected
    ...
    def resetIDs(self):
        ...
        sql = "UPDATE perawat SET id = @count:= @count + 1;"
        self.conn.execute(sql)
        sql = "ALTER TABLE perawat AUTO_INCREMENT = 1;"
        self.conn.execute(sql)
        self.conn.disconnect()
